...
- **「リカバリの穴」への自動履歴フォールバック**: サーバー側の Outbox クリーンアップ（24時間）により必要な差分フィードが消去された古いカーソルを検知した際、自動的に永続データベースの直近履歴（スナップショット）を取得してクライアントへ送り、重複排除（`id` によるマージ）で安全に最新状態へ追いつくフォールバック機構。
- **ダイレクトリクエスト（DM）のユーザー個別シーケンス化**: DM などのメッセージの採番をユーザー個別（`direct_request:{username}`）に行うことで、他のユーザーの送受信によるシーケンス番号の飛び番の誤検知を解消し、不要な REST 同期リクエストのオーバーヘッドを大幅に削減。
...
> [!NOTE]
> データ同期・復旧やロストゼロ設計の具体的なフローやシーケンス図は、[データ受信・復旧アーキテクチャ設計書 (docs/data-receiving-architecture.md)](file:///c:/Users/kanon/Desktop/MyProjects/Learn/WebSockets/fastapi-nextjs-websocket/docs/data-receiving-architecture.md) をご参照ください。

### 3.2 パフォーマンスと効率（超低負荷 & スケーラビリティ）
- **LISTEN / NOTIFY**: PostgreSQL の pub/sub 機能を利用。新着メッセージが DB に入った瞬間にリレープロセスへ合図を送り、高頻度なポーリングなしに低遅延配信を実現します。
- **SKIP LOCKED**: 複数サーバー（オートスケール）実行時も `FOR UPDATE SKIP LOCKED` により重複配信を防止し、スケーラビリティを確保します。
- **Redis Presence Store によるクラスタ全体の在席管理**: 複数インスタンスに接続が分散しても在席情報が分裂しないよう、Redis の Sorted Set で在席接続数を管理。TTL（有効期限）設定により異常終了時のゾンビ在席化を防止。
- **WebSocket 接続の切断・オーファンリーク防止**: フロントエンドの切断処理をアンマウント時（React 19 の StrictMode による二重マウント時を含む）に確実に実行し、サーバー側が即座に LEAVE イベントを配信できるように改善。
- **イベント購読のチャーン防止**: `useWsSubscribe` のハンドラを `useRef` を用いて安定化し、ハンドラ変化に伴う購読の解除・再登録による、一瞬のメッセージ取りこぼしを完全に排除。
- **リカバリフィードの取得上限とページング**: 1リクエストあたりの取得件数に上限（500件）を設け、クライアント側で順次ループ取得するページング制御と指数バックオフ付き自動リトライを実装。
...
- **データベースによるリフレッシュトークン管理とRTR (Refresh Token Rotation)**:
  ...
  - ログアウト時には、そのセッションのリフレッシュトークンをDBから削除して即座に無効化します。加えて、ユーザー単位で全リフレッシュトークンを一括削除する無効化機構（`delete_by_user_id`／キルスイッチ）も実装済みで、将来のパスワード変更・アカウント削除時の強制ログアウトに利用できます。
  - 期限切れ（7日以上経過）したリフレッシュトークンがDBに蓄積し続けるのを防ぐため、バックグラウンドの定期クリーンアップワーカー（`cleanup_worker`）が1時間ごとに自動削除タスクを実行し、デッドレコードの蓄積を自動的に防ぎます。
- **BFFでのリフレッシュ処理の重複排除 (Deduplication)**:
  - 放置後の復帰時などに発生する複数の並行した API 同期リクエストによるリフレッシュ競合を避けるため、同一リフレッシュトークンに対する BFF 側での処理をインメモリの Map を用いて重複排除しています。これにより、使い捨てリフレッシュトークン（RTR）の競合による不意の 401 エラーを完全に防ぎます。
- **401 未ログイン時の自動クローンアップと画面遷移**:
  - セッションやリフレッシュトークンの期限が切れて無効になった際、BFF 側は 401 応答を返すとともにブラウザの Cookie をクリアします。また、フロントエンド側でも共通の `apiClient` が 401 を検知して `unauthorized` カスタムイベントを発火し、`useAuth` フックが React のセッション状態をサイレントにクリアしてログイン画面へと自動で遷移させます。
...
> [!NOTE]
> BFFのCookie暗号化やアクセストークン中継、WebSocket接続チケットのやり取りなどの詳細なシーケンスは、[BFF認証アーキテクチャ設計書 (docs/bff-auth-architecture.md)](file:///c:/Users/kanon/Desktop/MyProjects/Learn/WebSockets/fastapi-nextjs-websocket/docs/bff-auth-architecture.md) をご参照ください。
> また、リフレッシュトークン（RTR・DB無効化）の設計と実装計画の詳細は、[リフレッシュトークン仕様書 (docs/refresh-token-implementation-plan.md)](file:///c:/Users/kanon/Desktop/MyProjects/Learn/WebSockets/fastapi-nextjs-websocket/docs/refresh-token-implementation-plan.md) をご参照ください。
