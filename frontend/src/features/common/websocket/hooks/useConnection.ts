  ...
  const connectWs = useCallback(async () => {
    ...
    try {
      ...
      if (err instanceof UnauthorizedError) {
        // 認証エラー（401）の場合は、グローバルなログアウト処理（apiClientでトリガー済み）に
        // 任せるため、再接続タイマーのスケジュールやエラー表示を行わずに終了します。
        return;
      }
    ...
    socket.onclose = (event: CloseEvent) => {
      ...
      // クローズコード 1008 (Policy Violation / Session Revoked) を検知した場合、
      // ユーザーセッションが無効化されたため、再接続を行わず強制ログアウトを実行します。
      if (event.code === 1008) {
