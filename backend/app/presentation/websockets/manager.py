...
class ChatManager:
    ...
    def __init__(self) -> None:
        ...
        # セッション単位の強制切断（revoke_session）のため、各接続がどの
        # セッションに属するかを記録する。チケットに session_id が無い
        # （古いクライアント等）場合は None になり、全断対象としてのみ扱われる。
        self._session_by_ws: dict[WebSocket, SessionId | None] = {}
    ...
    def disconnect(self, websocket: WebSocket, user_id: UserId | None = None) -> None:
        ...
        self._session_by_ws.pop(websocket, None)
    ...
    async def force_disconnect_user(self, user_id: UserId) -> None:
        ...
        if user_id not in self.connections:
            logger.info(
                "force_disconnect_user: user %s is not connected. Skip.", user_id.value
            )
            return

        ws_set = list(self.connections[user_id])
        logger.info(
            "force_disconnect_user: closing %d connections for user %s",
            len(ws_set),
            user_id.value,
        )
        for ws in ws_set:
            try:
                # ユーザーセッションが無効化されたため、
                # クローズコード 1008 (Policy Violation) を使用
                await ws.close(code=1008, reason="session_revoked")
            except RuntimeError as e:
                logger.debug("force_disconnect_user error during close: %s", e)
            finally:
                self.disconnect(ws, user_id)
    ...
    async def force_disconnect_session(
        ...
        他デバイス・他セッションの接続には影響しない。session_id が無いまま
        接続された WebSocket（古いクライアント等）も対象外とする。
        """
        if user_id not in self.connections:
            logger.info(
                "force_disconnect_session: user %s is not connected. Skip.",
                user_id.value,
            )
            return
        ...
        ws_set = [
            ...
            if self._session_by_ws.get(ws) == session_id
        ]
        logger.info(
            "force_disconnect_session: closing %d connections for user %s session %s",
            len(ws_set),
            user_id.value,
            session_id.value,
        )
        for ws in ws_set:
            try:
                ...
                logger.debug("force_disconnect_session error during close: %s", e)
            finally:
                self.disconnect(ws, user_id)
    ...
    async def reconnect_user(self, user_id: UserId) -> None:
        ...
        if user_id not in self.connections:
            logger.info(
                "reconnect_user: user %s is not connected. Skip.", user_id.value
            )
            return

        ws_set = list(self.connections[user_id])
        logger.info(
            "reconnect_user: closing %d connections for user %s to prompt reconnect",
            len(ws_set),
            user_id.value,
        )
        for ws in ws_set:
            try:
                # クライアントにログアウトさせず、指数バックオフで自動再接続させるため、
                # クローズコード 1000 (Normal Closure) を使用
                await ws.close(code=1000, reason="reconnect")
            except RuntimeError as e:
                logger.debug("reconnect_user error during close: %s", e)
            finally:
                self.disconnect(ws, user_id)
