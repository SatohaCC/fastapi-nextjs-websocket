...
def get_redis_client() -> aioredis.Redis:
    """Redis クライアント（接続プール）のシングルトンを取得します。"""
...
async def get_access_token(
    ...
    """Authorization ヘッダーから AuthToken を抽出します。"""
...
async def get_ws_authenticated_user(
    ...
    # 2. チケットの検証
    consumed = await ticket_store.consume_ticket(ticket)
    if not consumed:
        raise WebSocketException(
            code=status.WS_1008_POLICY_VIOLATION, reason="Invalid or expired ticket"
        )
    user_id, session_id = consumed
    ...
    # セッション単位の強制切断（revoke_session）を実現するため、接続登録時に
    # 参照できるよう websocket.state へ橋渡しする。
