...
class UserAdmin(ModelView, model=UserORM):
    ...
    # username は messages/tasks の非正規化コピー更新や WebSocket 再接続の
    # パブリッシュ（AuthService.update_username）を伴うため、直接編集を禁止し
    # 正規のユースケース経由（PATCH /api/auth/me）に限定する。
    form_excluded_columns = ["hashed_password", "username"]  # type: ignore[list-item]
...
class RefreshTokenAdmin(ModelView, model=RefreshTokenORM):
    ...
    async def after_model_delete(self, model: RefreshTokenORM, request) -> None:
        """セッション削除後に WebSocket 強制切断イベントを
        パブリッシュします。
        """
        if model.user_id:
            try:
                ...
                await redis_client.publish("session_control", json.dumps(payload))
                logger.info(
                    "SQLAdmin: Published force_disconnect event for "
                    "user %s after model delete",
                    model.user_id,
                )
            except Exception as e:
                logger.error(
                    "SQLAdmin: Failed to publish force_disconnect event: %s", e
                )
