...
from ...domain.primitives.primitives import EntityId, TaskText, UserId
...
class SendDirectRequestRequest(BaseModel):
    ...
    def to_domain(self) -> tuple[UserId, TaskText]:
        """To / text フィールドをドメインプリミティブへ変換します。"""
        import uuid

        return UserId(uuid.UUID(self.to)), TaskText(self.text)


@router.get("")
async def get_direct_requests(
    ...
    """
    指定された条件に基づいて、自分に関連するダイレクトリクエストを取得します。
    """
    limit_val = limit if limit is not None else 50
    if after_id is not None:
        tasks = await direct_request_service.get_tasks_after(
            user.id, EntityId(after_id)
        )
    elif before_id is not None:
        tasks = await direct_request_service.get_tasks_before(
            user.id, EntityId(before_id), limit=limit_val
        )
    else:
        tasks = await direct_request_service.get_recent_tasks(user.id, limit=limit_val)
...
async def send_request(
    ...
    recipient_id, text = body.to_domain()
