...
ALICE = User(
    ...
    hashed_password=HashedPassword("$2b$x"),
)
BOB = User(
    id=BOB_ID,
    userid=Userid("bob"),
...
def setup_users_mock(mock_uow):
    """すべてのテストケース用のユーザーリポジトリのモック初期化処理。"""
    mock_uow.users.get_by_id.side_effect = lambda uid: (
        ALICE if uid == ALICE_ID else (BOB if uid == BOB_ID else None)
    )
    mock_uow.users.get_by_userid.side_effect = lambda uid: (
        ALICE if uid.value == "alice" else (BOB if uid.value == "bob" else None)
    )
...
class TestDirectRequestServiceSendRequest:
    ...
    async def test_saves_task_to_repository(self, mock_uow, saved_task):
        ...
        await service.send_request(
            ALICE_ID, Username("alice"), BOB_ID, TaskText("please review")
        )
    ...
    async def test_outbox_feed_payload_maps_entity_fields(self, mock_uow, saved_task):
        ...
        assert feed.payload.sender_userid == Userid("alice")
        assert feed.payload.recipient_userid == Userid("bob")
    ...
    async def test_recipient_not_found_raises(self, mock_uow):
        ...
        mock_uow.users.get_by_id.side_effect = None
        mock_uow.users.get_by_id.return_value = None
        service = DirectRequestService(mock_uow)
        with pytest.raises(EntityNotFoundException):
            await service.send_request(
                ALICE_ID, Username("alice"), UserId(uuid.uuid7()), TaskText("test")
            )
