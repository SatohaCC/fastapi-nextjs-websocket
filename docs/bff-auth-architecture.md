...
- **リフレッシュトークン対応とDB無効化（実装済み）**: 暗号化されたリフレッシュトークンを `bff_refresh` Cookie に保管し、401エラー検知時に自動で透過的リフレッシュを行います。トークンはデータベース（`refresh_tokens` テーブル）にハッシュ保存され、RTR（Refresh Token Rotation）のほか、ログアウト時のセッション即時無効化（キルスイッチ）が実装されています。
- **プロキシのホワイトリスト化**: `/api/proxy/[...path]` について、将来的に転送を許可するエンドポイントを限定（ホワイトリスト化）することでセキュリティレベルを高めることが可能です。
